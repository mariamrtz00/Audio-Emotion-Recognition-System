% Parto de P6 que hasta ahora ha dado los mejores resultados
...
% Guardr0 las etiquetas tanto de numeros naturales como one hot
...
for i = 1:size(combinedData, 2)
    ...
    if isempty(audio)
        ...
        % Verifica si el resultado es NaN porque si no me da error??
        if isnan(zcrValue)
             zcrMatrix(i) = 0; 
        else
...
allFeaturesMatrix = [chromaMatrix; f0Matrix; zcrMatrix; mfccMatrix; rmsMatrix];
...
net6z = patternnet(hiddenLayerSizes);
 
% Añado Softmax a todas las capas
for i = 1:length(net6z.layers)
    net6z.layers{i}.transferFcn = 'softmax';
end

% ajustamos manualmente la regularización L2
lambda = 0.01; % Parámetro de regularización L2
net6z.performParam.regularization = lambda;

% patternet por defecto utiliza logsis en las capas ocultas y softmax en la capa de salida!

% parámetros de entrenamiento
net6z.trainParam.epochs = 6000;  % Número de épocas
net6z.trainParam.goal = 0;  % Objetivo de rendimiento
net6z.trainParam.lr = 0.001;  % Tasa de aprendizaje
net6z.trainParam.min_grad = 1e-6; % Criterio de convergencia basado en el gradiente
net6z.trainParam.max_fail = 1000000000;  % Número máximo de fallos consecutivos


% Entreno la red neuronal
net6z = train(net6z, trainingFeatures', trainingLabelsFeatures');

% Guardo la red 
save('red_neuronal_entrenada.mat6z', 'net6z');




%% VALIDACIÓN EN EL CONJUNTO DE ENTRENAMIENTO

% Utilizo la red neuronal para predecir etiquetas
predictions = net6z(trainingFeatures');
...
% Calcula la matriz de confusión en el conjunto de entrenamiento
confusionMatrix = confusionmat(trueLabels, predictedLabels);


% Creo una figura de la matriz de confusión NORMALIZADA
figure;
confusionchart(trueLabels, predictedLabels, 'Normalization', 'row-normalized');
title('Matriz de Confusión Normalizada en el Conjunto de entrenamiento');


% Muestro por la pantalla de comandos la matriz de confusión 
disp('Matriz de Confusión en el conjunto de entrenamiento:');
disp(confusionMatrix);

% Calculo la precisión 
precision = confusionMatrix(1,1) / (confusionMatrix(1,1) + confusionMatrix(2,1) + confusionMatrix2(3,1) + confusionMatrix(4,1) + confusionMatrix(5,1) + confusionMatrix(6,1) + confusionMatrix(7,1)+ confusionMatrix(8,1));

precisionPorcentaje = precision * 100;
disp(['Precisión en el conjunto de entrenamiento: ' num2str(precisionPorcentaje) '%']);





%% VALIDACIÓN EN EL CONJUNTO DE VALIDACIÓN EXTRA

% Utilizo la red neuronal para predecir etiquetas
predictions2 = net6z(featuresEvaluation);
...
% Calcula la matriz de confusión en el conjunto de VALIDACION EXXTRA
confusionMatrix2 = confusionmat(trueLabels2, predictedLabels2);

% Creo una figura de la matriz de confusión NORMALIZADA
figure;
confusionchart(trueLabels2, predictedLabels2, 'Normalization', 'row-normalized');
...
disp(confusionMatrix2);

% Calculo la precisión
precision2 = confusionMatrix2(1,1) / (confusionMatrix2(1,1) + confusionMatrix2(2,1) + confusionMatrix2(3,1) + confusionMatrix2(4,1) + confusionMatrix2(5,1) + confusionMatrix2(6,1) + confusionMatrix2(7,1)+ confusionMatrix2(8,1));

precisionPorcentaje2 = precision2 * 100;
disp(['Precisión en el conjunto de validación extra: ' num2str(precisionPorcentaje2) '%']);
...
% Utilizo la red neuronal para predecir etiquetas
predictions3 = net6z(validationFeatures');
predictedLabels3 = vec2ind(predictions3)';

% etiquetas reales del conjunto de validación
trueLabels3 = vec2ind(validationLabelsFeatures')';

% Calcula la matriz de confusión en el conjunto de validación
confusionMatrix3 = confusionmat(trueLabels3, predictedLabels3);

% Creo una figura de la matriz de confusión NORMALIZADA
figure;
confusionchart(trueLabels3, predictedLabels3, 'Normalization', 'row-normalized');
title('Matriz de Confusión Normalizada en el Conjunto de Validación');

% Muestro por la pantalla de comandos la matriz de confusión 
disp('Matriz de Confusión en el conjunto de validación:');
disp(confusionMatrix3);

% Calculo la precisión
precision3 = confusionMatrix3(1,1) / (confusionMatrix3(1,1) + confusionMatrix3(2,1) + confusionMatrix3(3,1) + confusionMatrix3(4,1) + confusionMatrix3(5,1) + confusionMatrix3(6,1) + confusionMatrix3(7,1)+ confusionMatrix3(8,1));

precisionPorcentaje3 = precision3 * 100;
disp(['Precisión en el conjunto de validación extra: ' num2str(precisionPorcentaje3) '%']);

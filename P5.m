% utiliza el mismo código que P4, pero sustituye la regularización L2 por la regularización L1.
...
net5 = patternnet(hiddenLayerSizes);

net5.layers{end}.size = 8; % Tamaño de la salida (8 en este caso)

% Parámetro de regularización L1
lambda = 0.01;

% parámetros de entrenamiento
net5.trainParam.epochs = 1000;  % Número de épocas
net5.trainParam.goal = 0;  % Objetivo de rendimiento
net5.trainParam.lr = 0.001;  % Tasa de aprendizaje
net5.trainParam.min_grad = 1e-6; % Criterio de convergencia basado en el gradiente
net5.trainParam.max_fail = 1000;  % Número máximo de fallos consecutivos

% Entreno la red neuronal
net5 = train(net5, trainingFeatures', trainingLabelsFeatures');

% Aplico regularización L1 a los pesos manualmente
for i = 1:length(net5.IW)
    net5.IW{i} = net5.IW{i} - sign(net5.IW{i}) * lambda;
end

for i = 1:length(net5.LW)
    net5.LW{i, i + 1} = net5.LW{i, i + 1} - sign(net5.LW{i, i + 1}) * lambda;
end

% Guardo la red 
save('red_neuronal_entrenada5.mat', 'net5');



%% Las gráficas matriz de confusión y performance aparecen durante el entrenamiento de la red
...
predictions = net5(trainingFeatures');
...
% Utilizo la red neuronal para predecir etiquetas
predictions2 = net5(featuresEvaluation);
...
% Utilizo la red neuronal para predecir etiquetas
predictions = net5(validationFeatures');
predictedLabels = vec2ind(predictions)';

...
cv = cvpartition(totalSamplesFeatures, 'KFold', 5); % 5-fold cross-validation
...
for fold = 1:cv.NumTestSets
    ...
    % Obtén conjuntos de entrenamiento y prueba
    trainingFeatures = normalizedFeaturesMatrix(trainingIndices, :);
    trainingLabelsFeatures = combinedLabels(trainingIndices, :);

    testFeatures = normalizedFeaturesMatrix(testIndices, :);
    testLabelsFeatures = combinedLabels(testIndices, :);

    % Crea y entrena la red neuronal para cada fold
    nett = patternnet(hiddenLayerSizes);
    nett.layers{end}.size = 8;
    nett.performParam.regularization = lambda; % Regularización L2
    nett.trainParam.epochs = 100;
    nett.trainParam.goal = 0;
    nett.trainParam.lr = 0.001;
    nett.trainParam.min_grad = 1e-6;
    nett.trainParam.max_fail = 1000000000;
    nett = train(nett, trainingFeatures', trainingLabelsFeatures');
    % Guarda la red neuronal después de entrenar
    save(['net_fold_' num2str(fold) '.mat'], 'nett');
    % Evalúa la red neuronal en el conjunto de prueba
    predictions = nett(testFeatures');
    predictedLabels = vec2ind(predictions);

    % Calcula y muestra la precisión para este fold
    trueLabels = vec2ind(testLabelsFeatures');
    accuracy = sum(predictedLabels == trueLabels) / length(trueLabels) * 100;
    disp(['Fold ' num2str(fold) ': Porcentaje de emociones correctas detectadas en el conjunto de prueba: ' num2str(accuracy) '%']);
end
...
predictions = nett(trainingFeatures');
predictedLabels = vec2ind(predictions)';
...
% Utilizo la red neuronal para predecir etiquetas
predictions = nett(validationFeatures');
predictedLabels = vec2ind(predictions)';
...
% Utilizo la red neuronal para predecir etiquetas
predictions2 = nett(featuresEvaluation);
...
output = net6(trainingFeatures');

% Convertir las salidas a etiquetas (0 o 1) basadas en un umbral (por ejemplo, 0.5)
threshold = 0.5;
predictedLabels = output > threshold;


% Calcular la precisión
accuracy = sum(predictedLabels == trainingLabelsFeatures') / numel(trainingLabelsFeatures);

disp(['Precisión en el conjunto de prueba: ' num2str(accuracy * 100) '%']);
